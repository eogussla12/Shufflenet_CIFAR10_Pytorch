...
if __name__ == '__main__':
    ...
    checkpoint_path = os.path.join(settings.CHECKPOINT_PATH, "shufflenet", settings.TIME_NOW)

    #use tensorboard
    if not os.path.exists(settings.LOG_DIR):
        os.mkdir(settings.LOG_DIR)
    writer = SummaryWriter(log_dir=os.path.join(
            settings.LOG_DIR, "shufflenet", settings.TIME_NOW))
    ...
    for epoch in range(1, settings.EPOCH):
        ...
        if epoch > settings.MILESTONES[1] and best_acc < acc:
            torch.save(net.state_dict(), checkpoint_path.format(net=shufflenet, epoch=epoch, type='best'))
            best_acc = acc
            continue

        if not epoch % settings.SAVE_EPOCH:
            torch.save(net.state_dict(), checkpoint_path.format(net=shufflenet, epoch=epoch, type='regular'))
